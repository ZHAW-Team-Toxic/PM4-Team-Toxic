    ...
    private static JSONObject findBestMatchingRelease(String platformTag) throws IOException, InterruptedException {
        ...
        for (int i = 0; i < releases.length(); i++) {
            ...
            for (int j = 0; j < assets.length(); j++) {
                ...
                if (name.contains(platformTag.toLowerCase()) && name.endsWith(".zip")) {
                    System.out.println("Using release: " + release.optString("tag_name", "<no-tag>"));
                    return release;
                }
            }
        }

        throw new IOException("No release (including prereleases) contained a matching ZIP for platform: " + platformTag);
    }
